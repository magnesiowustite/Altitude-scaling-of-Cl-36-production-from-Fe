function N = Neutrons_mod(h,Rc,s,w,consts,nuclide)

%The following is modified from the "Neutrons" code in Lifton et al. (2014)
%to consider neutron spallation production of Cl-36 on all major Cl-36 target
%elements (i.e., Fe, Ti, Ca, and K), Be-10 in quartz, and the integral neutron
%flux.
...
for a = 1:length(Rc)
    ...
    elseif nuclide == 56 %Fe
        N.PFe36n(a) = trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.FenxCl36(clipindex:end))*6.022E23/55.845*1e-27.*3.1536e7;
    elseif nuclide == 48 %Ti
        N.PTi36n(a) = trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.TinxCl36(clipindex:end))*6.022E23/47.87*1e-27.*3.1536e7;
    elseif nuclide == 39 %K
        N.PK36n(a) = trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.KnxCl36(clipindex:end))*6.022E23/39.10*1e-27.*3.1536e7;
    elseif nuclide == 40 %Ca
        N.PCa36n(a) = trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.CanapCl36(clipindex:end))*6.022E23/40.08*1e-27.*3.1536e7;
    else %Integral flux
