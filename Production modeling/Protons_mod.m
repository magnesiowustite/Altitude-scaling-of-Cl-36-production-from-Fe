function P = Protons_mod(h,Rc,s,consts,nuclide)

%The following is modified from the "Protons" code in Lifton et al. (2014)
%to consider proton spallation production of Cl-36 on all major Cl-36 target
%elements (i.e., Fe, Ti, Ca, and K), Be-10 in quartz, and the integral proton
%flux.
...
for a = 1:length(Rc)
    ...
    if  nuclide == 10 %Be-10 in qtz
        P.P10p(a) = (trapz(E,phiPtot.*consts.O16pxBe10) + trapz(E,phiPtot.*consts.SipxBe10./2)).*consts.Natoms10.*1e-27.*3.1536e7;
    elseif nuclide == 56 %Fe
        P.PFe36p(a) = trapz(E,phiPtot.*consts.FepxCl36)*6.022E23/55.845*1e-27.*3.1536e7;
    elseif nuclide == 48 %Ti
        P.PTi36p(a) = trapz(E,phiPtot.*consts.TipxCl36)*6.022E23/47.87*1e-27.*3.1536e7;
    elseif nuclide == 39 %K
        P.PK36p(a) = trapz(E,phiPtot.*consts.KpxCl36)*6.022E23/39.10*1e-27.*3.1536e7;
    elseif nuclide == 40 %Ca
        P.PCa36p(a) = trapz(E,phiPtot.*consts.CapxCl36)*6.022E23/40.08*1e-27.*3.1536e7;
    else %Integral flux
